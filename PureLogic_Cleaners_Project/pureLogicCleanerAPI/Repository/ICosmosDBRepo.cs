...
namespace pureLogicCleanerAPI.Repository
{
    public interface ICosmosDBRepo
    {
        ...
        Task<bool> UpdateAsync<T>(T data, string containerName, string partitionKey) where T : class;
        Task<bool> DeleteAsync<T>(string containerName, string partitionKey) where T : class;
        void SetAsync();
    }
}
